...
<div class="w-100 row px-3">
    ...
    <div class="my-1 p-1 col-4">
        <div class="caja_producto text-white" style="background-color: rgba(0,0,0,.5);">
            ...
            <form action="./accion/accionAgregarAlCarrito.php" method="POST" class="needs-validation Comprar"
                novalidate>
                <input type="text" id="idUsuario" name="idUsuario"
                    value="<?= $objSession->getUsuario()->getIdUsuario() ?>" hidden>
                <input type="text" name="idProducto" id="idProducto" class="d-none"
                    value="<?= $objProducto->getIdProducto() ?>">
                <div class='container-fluid'>
                    <div class='d-inline-block'>
                        <input type="number" value='0' name="ciCantidad" id="cantidad_input" min="1"
                            max="<?= $objProducto->getProCantStock() ?>" class="form-control col-sm-2"
                            placeholder="cant" required cols="2" width='60px'
                            <?= $objProducto->getProCantStock() > 0 ? "" : "disabled" ?>>
                    </div>
...
<div class="col-12 d-flex align-items-center"
    style="align-items: end; padding-right: 30px; margin-top: 10px; margin-bottom: 10px; justify-content: space-between; text-align: center;">
    <h1>CARRITO</h1>
