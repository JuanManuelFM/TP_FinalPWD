...
function mostrarAdvertencia(form){
    Swal.fire({
        title: '¿Está seguro sobre deshabilitar al usuario?',
        showDenyButton: true,
...
function deshabilitarUsuario(form){
    $.ajax({
        type: "POST",
        url: 'accion/usuarioHabilitacion.php',
        data: form.serialize(),
...
function deshabilitarSuccess(){
    Swal.fire({
        icon: 'success',
        title: 'Se deshabilito al usuario correctamente!',
...
function deshabilitarFailure(){
    Swal.fire({
        icon: 'error',
        title: 'No se pudo deshabilitar al usuario',
        showConfirmButton: false,
        timer: 1500
    })
}
...
function mostrarAdvertenciaHabilitar(form){
    Swal.fire({
        title: '¿Está seguro sobre habilitar al usuario?',
        showDenyButton: true,
        confirmButtonText: 'Si',
        denyButtonText: `Cancelar`,
      }).then((result) => {
...
function habilitarSuccess(){
    Swal.fire({
        icon: 'success',
        title: 'Se ha habilitado el usuario correctamente!',
        showConfirmButton: false,
        timer: 1500
    })
    setTimeout(function(){
